...
<style>
:root {
  --bg:#f7f4f2; --surface:#fff; --surface2:#f2eee9; --surface3:#ede7e0;
  --border:#e8e0d8; --border2:#d4c9bf;
  --text:#1a1714; --text2:#4a3f38; --muted:#8c7d73; --muted2:#b5a99f;
...
.sidebar-logo{padding:0 24px 32px;font-family:var(--fd);font-size:24px;color:var(--text)}
...
/* ── BOUTONS ── */
.btn{display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:var(--radius-sm);font-family:var(--fb);font-size:13px;font-weight:700;cursor:pointer;border:none;transition:all .18s;text-decoration:none;white-space:nowrap}
...
.page-header h1{font-family:var(--fd);font-size:34px;color:var(--text)}
...
.kpi-val{font-family:var(--fd);font-size:30px;font-weight:700;color:var(--text)}
...
.fi{background:var(--surface);border:1.5px solid var(--border);border-radius:8px;padding:9px 13px;color:var(--text);font-family:var(--fb);font-size:13px;transition:border-color .18s;width:100%}
...
/* ── TEINTES ── */
.sh-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:9px;margin-bottom:18px}
...
.sh-sw{width:32px;height:32px;border-radius:7px;flex-shrink:0;border:2px solid rgba(0,0,0,.08)}
.sh-img{width:32px;height:32px;border-radius:7px;object-fit:cover;border:1px solid var(--border);flex-shrink:0}
...
.sh-form{background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius-sm);padding:18px;margin-top:4px}
...
.sh-img-preview img{width:40px;height:40px;object-fit:cover;border-radius:6px;border:1px solid var(--border)}
...
.modal{background:var(--surface);border:1px solid var(--border);border-radius:22px;width:100%;max-width:820px;margin:0 auto;position:relative;box-shadow:var(--shadow)}
...
.m-title{font-family:var(--fd);font-size:26px;font-weight:700;color:var(--text)}
...
.tab-btn{padding:12px 18px;font-size:13px;font-weight:700;color:var(--muted);cursor:pointer;border:none;background:none;border-bottom:2px solid transparent;margin-bottom:-1px;transition:all .18s;display:flex;align-items:center;gap:7px;font-family:var(--fb)}
...
.m-sh-row{display:grid;grid-template-columns:36px 1fr auto auto auto;gap:8px;align-items:center;background:var(--surface2);border:1px solid var(--border);border-radius:9px;padding:9px 12px}
.m-sh-thumb{width:32px;height:32px;border-radius:6px;object-fit:cover;border:1px solid var(--border)}
.m-sh-sw{width:32px;height:32px;border-radius:6px;border:2px solid rgba(0,0,0,.08);flex-shrink:0}
...
.shade-file-strip .sf-item{position:relative;width:54px}
.shade-file-strip .sf-item img{width:54px;height:54px;object-fit:cover;border-radius:6px;border:1px solid var(--border)}
...
@media(max-width:960px){
  ...
  .m-sh-row{grid-template-columns:36px 1fr auto auto}
}
</style>
</head>
<body>
...
<div class="edit-layout">
      ...
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-2" style="margin-bottom:14px">
          ...
          <div class="fg">
            ...
            <input type="number" name="old_price" class="fi" step="0.01" min="0"
                   value="<?= $editProduct['old_price'] ?? '' ?>" placeholder="Barré si > prix">
          </div>
          ...
          <div class="fg full">
            ...
            <div style="margin-top:9px;display:flex;align-items:center;gap:10px">
              <img src="<?= htmlspecialchars(imgUrl($b, $editProduct['image_url'])) ?>"
                   style="height:52px;border-radius:8px;border:1px solid var(--border)" alt="">
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap">
          ...
          <a href="admin_products.php?delete=<?= $editProduct['id'] ?>"
  ...
  <div id="teintes">
          ...
          <div class="sh-grid">
            ...
            <div class="sh-card">
              ...
              <div class="sh-info">
                ...
                <div class="sh-meta">
                  Stock : <?= (int)($t['stock'] ?? 0) ?>
                  <?php if (!empty($t['prix']) && $t['prix'] > 0): ?>
                    · <?= number_format((float)$t['prix'], 2, ',', ' ') ?> DA
                  <?php endif; ?>
                  <?php if ($tImgUrl): ?>
              ...
              <div class="sh-acts">
                <button class="btn btn-ghost btn-sm" onclick='loadShade(<?= json_encode($t, JSON_HEX_APOS|JSON_HEX_QUOT) ?>)' title="Modifier">✎</button>
                <a href="admin_products.php?delete_shade=<?= $t['id'] ?>&pid=<?= $editProduct['id'] ?>"
                   class="btn btn-danger btn-sm"
                   onclick="return confirm('Supprimer cette teinte et son image ?')" title="Supprimer">✕</a>
              </div>
            </div>
            <?php endforeach; ?>
          </div>
            ...
            <form method="POST" enctype="multipart/form-data">
              ...
              <div class="form-3">
                ...
                <div class="fg">
                  ...
                  <input type="number" name="stock_shade" id="shStk" class="fi" min="0"
                         value="<?= $editShade ? (int)$editShade['stock'] : 0 ?>">
                </div>

                <div class="fg">
                  <label class="fl">Prix spécifique (DA)</label>
                  <input type="number" name="prix_shade" id="shPrix" class="fi" step="0.01" min="0"
                         value="<?= $editShade ? (float)($editShade['prix'] ?? 0) : 0 ?>"
                         placeholder="0 = prix produit">
                </div>

                <div class="fg" style="grid-column:span 2">
                  <label class="fl">Image de la teinte</label>
                  <div class="upzone" style="padding:14px">
                    <input type="file" name="shade_image" accept="image/*"
                           onchange="previewImg(this,'shImgPrev')">
                    <div class="up-icon" style="font-size:18px">🎨</div>
                    <div class="up-txt">Photo associée à cette teinte · JPG PNG WEBP</div>
                    <img id="shImgPrev" class="up-prev" alt="">
                  </div>
                  <?php if ($editShade && !empty($editShade['image'])): ?>
                  <div class="sh-img-preview" id="shCurrentImgWrap">
                    <img src="<?= htmlspecialchars(imgUrl($b, $editShade['image'])) ?>" alt="Image actuelle">
              ...
              <div style="margin-top:14px;display:flex;gap:9px">
                <button type="submit" class="btn btn-primary btn-sm" id="shBtn">
                  <?= $editShade ? '✦ Mettre à jour' : '✦ Ajouter' ?>
                </button>
    ...
    <div class="info-box">
      ...
      L'image de chaque teinte remplace l'image principale sur <code>product.php</code> quand la teinte est sélectionnée.
    </div>
  </div>
</div>
    ...
    <div class="gal-grid">
      ...
      <div class="gal-item">
        ...
        <a href="admin_products.php?delete_img=<?= $img['id'] ?>&pid=<?= $editProduct['id'] ?>"
           class="gal-del" onclick="return confirm('Supprimer cette image ?')">✕</a>
      </div>
      <?php endforeach; ?>
    </div>
    <?php else: ?>
          ...
          <tr>
            <td>
              <?php if (!empty($p['image_url'])): ?>
                <img src="<?= htmlspecialchars(imgUrl($b, $p['image_url'])) ?>"
                     class="pimg" alt="" onerror="this.style.display='none'">
              <?php else: ?>
            ...
            <td>
              <span class="bdg <?= $p['active'] ? 'bdg-ok' : 'bdg-off' ?>">
            ...
            <td>
              <div class="acts">
                ...
                <a href="admin_products.php?delete=<?= $p['id'] ?>"
                   class="btn btn-danger btn-sm"
                   onclick="return confirm('Supprimer ce produit ?')">✕</a>
              </div>
            </td>
          </tr>
          <?php endforeach; ?>
    ...
    <form method="POST" enctype="multipart/form-data" id="createForm">
      ...
      <div class="tab-panel active" id="t-info">
        <div class="form-2" style="gap:14px">
          ...
          <div class="fg">
            <label class="fl">Ancien prix (DA)</label>
            <input type="number" name="old_price" class="fi" step="0.01" min="0" placeholder="Barré sur la fiche">
          </div>
          <div class="fg">
          ...
          <div class="fg full" style="gap:11px">
            ...
            <label class="tog">
              ...
              <span class="tog-lbl">Produit avec teintes — ajoutez-les dans l'onglet 🎨</span>
            </label>
            <label class="tog">
              <input type="checkbox" name="active" value="1" checked>
              <span class="tog-track"></span>
              <span class="tog-lbl">Actif dès la création</span>
            </label>
          </div>
          <div class="fg full">
        ...
        <div style="background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius-sm);padding:16px;margin-top:4px">
          ...
          <div class="form-3">
            ...
            <div class="fg">
              <label class="fl">Couleur</label>
              <div class="col-row">
                <input type="color" id="mShCol" class="fi" value="#c4697a"
                       style="padding:3px;height:38px;cursor:pointer"
                       oninput="document.getElementById('mColSw').style.background=this.value">
            ...
            <div class="fg">
              <label class="fl">Prix (DA)</label>
              <input type="number" id="mShPrix" class="fi" step="0.01" value="0" min="0" placeholder="0 = prix produit">
            </div>
            <div class="fg" style="grid-column:span 2">
              <label class="fl">Image de la teinte</label>
      ...
      <div class="tab-panel" id="t-images">
        ...
        <p style="font-size:12px;color:var(--muted);margin-bottom:14px">
          Ces images seront affichées comme miniatures sur la page produit.
        </p>
        <div class="gal-zone">
          <input type="file" name="extra_images[]" accept="image/*" multiple
                 onchange="previewGalleryModal(this)">
          <div class="up-icon">🖼️</div>
...
function addShade() {
  ...
  const hasImg = !!(fileInput && fileInput.files[0]);
...
function removeShade(i) {
  ...
  Object.entries(shadeFiles).forEach(([k, v]) => {
    const ki = parseInt(k);
    if      (ki < i)  newFiles[ki]     = v;
    else if (ki > i)  newFiles[ki - 1] = v;
  });
  shadeFiles = newFiles;
  renderShades();
...
function renderShades() {
  ...
  list.innerHTML = shades.map((s, i) => {
    ...
    const prixTxt = s.prix ? ` · ${s.prix} DA` : '';
    return `<div class="m-sh-row">
      ${imgTag}
      <span class="m-sh-nm">${s.nom}</span>
      <span class="m-sh-stk">Stock : ${s.stock}${prixTxt}</span>
  ...
  Object.entries(shadeFiles).forEach(([k, file]) => {
    ...
    span.textContent = shades[parseInt(k)]?.nom || '';
    div.appendChild(img);
...
function loadShade(sh) {
  ...
  if (wrap) {
    if (sh.image) {
      ...
      if (existingImg && sh.image_url_full) existingImg.src = sh.image_url_full;
